...
export default function Portfolio({ store }: Props) {
  ...
  function getContractStart(p: Property) {
    const active = contracts.filter(c => c.propertyId === p.id && c.status === 'active').sort((a, b) => a.startDate.localeCompare(b.startDate))
          ...
          <tbody>
            {properties.map(p => (
              ...
              >
                ...
                <td className="px-4 py-3 text-gray-600">{fmtDate(getContractStart(p))}</td>
